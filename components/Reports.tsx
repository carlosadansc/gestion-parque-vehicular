...
const Reports: React.FC<ReportsProps> = ({ vehicles, fuelEntries, maintenanceRecords, incidents, settings = [] }) => {
  ...
  const incidentTypeData = useMemo(() => {
    ...
    const mapLabels: any = { mechanical: 'Mecánica', traffic: 'Tránsito', accident: 'Accidente', theft: 'Robo' };
    ...
    <div className={containerClass}>
      ...
      {!previewMode ? (
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 no-print">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Reportes Dinámicos</h2>
            <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">Inteligencia de negocios y análisis de flota.</p>
          </div>
      ...
      <div id="report-printable" className={contentClass}>
        ...
        <div className={`${previewMode ? 'flex' : 'hidden print:flex'} justify-between items-center mb-8 border-b-4 border-slate-900 pb-6`}>
            ...
            <div className="flex flex-col">
                ...
                <span className="text-sm font-bold uppercase text-slate-500 mt-1 tracking-widest">Sistema DIF Municipal La Paz</span>
            </div>
            </div>
            <div className="text-right">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Periodo del Reporte</p>
        ...
        <div className="space-y-8">
            ...
            <div className="break-inside-avoid">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-4 border-b border-slate-200 pb-2">Desglose Financiero Detallado</h3>
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-100 text-slate-600 border border-slate-200">
                            <th className="p-2 text-[9px] font-black uppercase">Vehículo</th>
                            <th className="p-2 text-[9px] font-black uppercase text-right">Combustible</th>
                            <th className="p-2 text-[9px] font-black uppercase text-right">Mantenimiento</th>
                            <th className="p-2 text-[9px] font-black uppercase text-right">Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        {costByVehicleData.map((item, idx) => (
                            <tr key={idx} className="border border-slate-200">
                                <td className="p-2 text-[10px] font-bold">{item.name}</td>
                                <td className="p-2 text-[10px] text-right font-mono">${item.fuel.toLocaleString()}</td>
                                <td className="p-2 text-[10px] text-right font-mono">${item.maintenance.toLocaleString()}</td>
                                <td className="p-2 text-[10px] text-right font-black font-mono">${(item.fuel + item.maintenance).toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
        ...
        {/* FOOTER */}
        <div className={`${previewMode ? 'block' : 'hidden print:block'} mt-12 pt-4 border-t border-slate-200 text-center`}>
            <p className="text-[8pt] font-black text-slate-300 uppercase tracking-[0.3em]">Sistema de Control Flota Pro • Generado Automáticamente</p>
        </div>
